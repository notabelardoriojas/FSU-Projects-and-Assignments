...
public:
  ...
  float ODE (float initial, float final, float yN) {
    float timestep = (final - initial) / N;
    ofstream myfile ("plot.txt");
    int count = 0;
    for (float i = initial; i <= final; i += timestep) {
      yN = yN + timestep*g(yN,i);
      xvalue.push_back(i);
      yvalue.push_back(yN);
      myfile << xvalue[count] << " " << yvalue[count] << "\n";
      count++;
    }
...
int main() {
	...
	curve.g = g;
	curve.setN(1000);
	cout << "Yn is " << curve.ODE(1,5,10) << endl;
